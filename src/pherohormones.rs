...
impl Pherohormones {
    ...
    pub fn to_dot(&self, cycle: i32) -> String {
...
"##,
        ...
        // Add edges with visual encoding
        for edge in self.di_graph.edge_indices() {
            if let Some((source, target)) = self.di_graph.edge_endpoints(edge) {
                if let Some(&weight) = self.di_graph.edge_weight(edge) {
                    let t = (weight - min_weight) / (max_weight - min_weight).max(f64::EPSILON);
                    let width = (t * 1.0).clamp(1.0, 1.0);
                    let hue = (t * 240.0) as u8; // Blue to red gradient
    ...
    subgraph cluster_legend {
        ...
        Legend [shape=plaintext, label=<
            <TABLE BORDER="0" CELLBORDER="0" CELLSPACING="4">
                <TR><TD COLSPAN="3"><FONT POINT-SIZE="10">Edge Strength</FONT></TD></TR>
                <TR>
                    <TD WIDTH="40"><FONT POINT-SIZE="8">Weak</FONT></TD>
                    <TD WIDTH="60" HEIGHT="10" BGCOLOR="#0000FF"></TD>
                    <TD WIDTH="40"></TD>
                </TR>
                <TR>
                    <TD></TD>
                ...
                <TR>
                    <TD><FONT POINT-SIZE="8">Strong</FONT></TD>
                    <TD WIDTH="60" HEIGHT="20" BGCOLOR="#FF0000"></TD>
                    <TD WIDTH="40"></TD>
                </TR>
            </TABLE>
        >];
...
"##,
    ...
    pub fn save_frame(&self, iteration: i32) -> std::io::Result<()> {
        fs::create_dir_all(&self.output_dir)?;
        let path = format!("{}/frame_{:04}.dot", self.output_dir, iteration);
        let mut file = File::create(&path)?;
        file.write_all(self.to_dot(iteration).as_bytes())?;

        // Auto-render to PNG if feature enabled
        #[cfg(feature = "auto_render")]
        {
            let output_png = path.replace(".dot", ".png");
            ...
            if !status.success() {
                eprintln!("Warning: Failed to render frame {}", cycle);
            }
        }
